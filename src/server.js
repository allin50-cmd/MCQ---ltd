  ...
  try{
    ...
    if(req.method==="POST"&&u.pathname==="/api/admin/crm/promote-hire"){
      ...
      const enquiry={
        ...
        guest_count:String(x.guest_count||lead.guest_count||"").trim(),
        playback:String(x.playback||lead.playback||"").trim(),
        budget:String(x.budget||lead.budget||"").trim(),
        notes:String(x.notes||lead.message||"").trim(),
        campaign_context:String(lead.campaign_context||"").trim(),
        utm_source:String(lead.utm_source||"").trim(),
        utm_medium:String(lead.utm_medium||"").trim(),
        utm_campaign:String(lead.utm_campaign||"").trim(),
        landing_url:String(lead.landing_url||"").trim(),
        equipment_id:eq.id,
        start_at:startAt,
        end_at:endAt,
        status:"NEW",
