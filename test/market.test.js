...
test("public-market research is publishable for sourcing but remains non-sellable until commercial evidence exists",()=>{
  assert.ok(MARKET_CATALOG.length>=7);
  const evaluated=listMarketCatalog();
  for(const row of evaluated){
    assert.equal(row.evidence_state,"PUBLIC_MARKET");
    assert.equal(row.state,"SOURCE_THROUGH_MCQ");
    assert.equal(row.public_display,true);
    ...
    assert.equal(row.validation.common_errors.length,0);
    assert.ok(row.validation.sellable_errors.length>0);
  }
  assert.ok(searchMarketCatalog("DDJ FLX4").length>=1);
});
